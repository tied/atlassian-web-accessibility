/**
 * @module confluence-editor/oninit/tinymce-cursor-fixes
 */
define('confluence-editor/oninit/tinymce-cursor-fixes', [
    ...
    function isCursorAtStart(ed, rng, selectedNode) {
        ...
        } if ($(startContainer).is('p:first-child')) { // we occasionally get a range with the startContainer being a P with offset 0 when the cursor is inside <p><br/></p>
            return rng.startOffset === 0;
        } if ($(startContainer).is('.wysiwyg-macro-body')) { // the first block element inside a macro body placeholder is always preceded by a textnode (containing a newline character) in FF and Webkit
    ...
    function isEmptyParagraph($p) {
        ...
        if (contents.length === 1) {
            if (contents.is('br')) {
                return true;
            } if (contents.html() === '&nbsp;' || contents.text().charCodeAt(0) === 160) {
                return true;
            } if (contents[0].nodeType === 3 && contents[0].nodeValue === AJS.Rte.HIDDEN_CHAR) {
                return true;
            }
        }
        return false;
