/**
 * @module confluence-editor/tinymce3/plugins/customtoolbar/chunky-tips
 */
define('confluence-editor/tinymce3/plugins/customtoolbar/chunky-tips', [
    ...
    function ChunkyTips() {
        ...
        $(document).tooltip(
            $.extend(
                {},
                tooltipDefaultBelowOptions,
                { live: '#rte-toolbar [data-tooltip]:not(.dropdown-item)' })
        );

        $(document).tooltip(
            $.extend(
                {},
                tooltipDefaultToRightOptions,
                { live: '#rte-toolbar .dropdown-item[data-tooltip]' })
        );

        $(document).tooltip(
            $.extend(
                {},
                tooltipDefaultAboveOptions,
                { live: '#rte-savebar [data-tooltip]' })
        );
        ...
        $(document).tooltip(
            $.extend(
                {},
                tooltipDefaultBelowOptions,
                { live: '#editor-precursor [data-tooltip]' })
        );
        ...
        $(document).tooltip(
            $.extend(
                {},
                tooltipDefaultBelowOptions,
                { live: '#property-panel [data-tooltip]' })
        );
