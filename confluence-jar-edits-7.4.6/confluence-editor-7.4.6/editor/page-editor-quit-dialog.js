...
define('confluence-editor/editor/page-editor-quit-dialog', [
    ...
    function init(options) {
        ...
        if (needDialog) {
            if (!isInitialized) {
                ...
                // saveExitButton is no more in use and has to be deleted from
                // - here and templates
                // - karma tests
                // - page objects
                // It was in use till Confluence 6.9.
                ...
                // Please refer to the jsdoc in synchrony-handlers.ls in collab-editor plugin (handleError function) for
                // more details about how eviction errors are processed on frontend
                AJS.bind('synchrony.history.evicted', function() {
                    unrecoverableEditorError = true;
                    $('#qed-discard-button').attr('disabled', 'disabled');
                });
                ...
                ExternalHandlers.cancelErrorHandler = options && options.cancelErrorHandler;
    ...
    function clickDiscard(e) {
        // we don't want to allow to discard draft, if it's outdated because of synchrony data cleanup.
        // In such case, draft's content in browser does not sync between editing sessions via synchrony, so we
        // don't want to update such drafts.
     ...
     * @param {object} e button click event OR jqXHR of draft save request
     */
    ...
    function clickRevertChanges(e) {
        e.stopPropagation();
        e.preventDefault();
        // we don't want to allow to show discard draft dialog, if it's outdated because of synchrony data cleanup.
        // In such case, draft's content in browser does not sync between editing sessions via synchrony, so we
        // don't want to update such drafts.
        // Please refer to the jsdoc in synchrony-handlers.ls in collab-editor plugin (handleError function) for more
        // details about how eviction errors are processed on frontend
    ...
    function process(e) {
        if (needDialog) {
            ...
            $(window).one('editor-heartbeat rte.heartbeat-error', function(ev, data) {
                $(this).off('editor-heartbeat rte.heartbeat-error');
                switch (e.target.id) {
    ...
    function processClose(ev) {
        Confluence.Editor.Drafts.save({
            ...
            onSuccessHandler: clickSaveExit,
            // sometimes synchrony declares local changes even before it is connected (CONFSRVDEV-10915)
            // to filter out such noise, we need synchronyConnected variable
            // Confluence.Editor.hasContentChanged() is needed to fix CONFSRVDEV-11167
            ignoreRelations: !(Confluence.Editor.hasContentChanged() && synchronyConnected)
        });
        var draftSavingPromise = Confluence.Editor.Drafts.getDraftSavingPromise();
        if (draftSavingPromise) {
            draftSavingPromise.fail(function(xhr) {
                // call external error handler if exists. Otherwise just rely on standard error handler
                // for shared drafts, this error handler triggers appropriate error message, which also aligns
                // with publish experience
                if (ExternalHandlers.cancelErrorHandler) {
                    ExternalHandlers.cancelErrorHandler(xhr);
                }
            });
        }
        // if heartbeat went through, it will re-enable publish and cancel buttons.
        // in order to avoid confusion, toggling savebar busy again (that what Publish button does as well)
        if (!ev.namespace || ev.namespace !== 'heartbeat-error') {
            Confluence.Editor.UI.toggleSavebarBusy(true);
        }
    }
