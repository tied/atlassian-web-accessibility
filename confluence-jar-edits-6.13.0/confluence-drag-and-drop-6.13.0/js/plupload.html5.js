...
(function(window, document, plupload, undef) {
	...
	plupload.runtimes.Html5 = plupload.addRuntime("html5", {
		...
		getFeatures : function() {
	        ...
	        // ATLASSIAN: CONFDEV-2126 disable drag and drop for Safari and Windows
            // ATLASSAIN: Safari 5 supports drag and drop
			fakeSafariDragDrop = plupload.ua.safari && plupload.ua.windows && navigator.userAgent && navigator.userAgent.indexOf("Version/4") > 0;
			...
			return {
                ...
                //CONFDEV-25763: Fix upload button in IE10
				triggerDialog: (plupload.ua.gecko && window.FormData || plupload.ua.webkit || plupload.ua.windows)
			};
		},
		...
		init : function(uploader, callback) {
			...
			function addSelectedFiles(native_files) {
				...
				for (i = 0; i < native_files.length; i++) {
                    ...
                    // ATLASSIAN: on firefox, the directory's size is 0
                    var pUploadFile = new plupload.File(id, file.fileName || file.name, file.fileSize || file.size);
                    pUploadFile.nativeFile = file;
                    files.push(pUploadFile); // fileName / fileSize deprecated
				}
			...
			/**
			 * ATLASSIAN - this is an Atlassian customisation and is used in image paste handling.
			 *
			 * Uploads an external file to confluence, supported file types are listed in UploadAction.
			 *
			 * The function optionally supports a base64 encoding of the file, this is specified by setting the encoding
			 * property of each file to base64.  The getData method on each external file should return the encoded representation
			 * of the file if the encoding is specified.
			 *
			 * The uploader can be referenced from AJS.Confluence.Uploader, see editor.js
			 *
			 * param externalFiles an array of "file" objects that implement the following interface:
			 *
			 * externalFile: {
			 *   fileName: 0,
			 *   fileSize: 0,
			 ...
			 *
			 */
            uploader.upload = function(externalFiles) {
                addSelectedFiles(externalFiles);
//                uploader.start();
            };
			...
			uploader.bind("Init", function(up) {
                ...
                //ATLASSIAN: add extra clazz to inputFile to get this input in webdriver test easier
                if (uploader.settings.inputFileClazz) {
                    inputFile.className += " " + uploader.settings.inputFileClazz;
                }
                ...
                var onInputChange = function() {
                    ...
                    // Atlassian - CONFDEV-27961
                    // Clearing the value enables the user to select the same file again if they want to
                    // Setting value of input to empty does not work on IE10
                    // so we need to replace it with cloned one from itself
                    if (navigator.appVersion.indexOf("MSIE10") === -1) {
                        inputFile = this.cloneNode(true);
                ...
                //CONFDEV-26930: allow browse_button is DOM element or id string of button
			...
			uploader.bind("PostInit", function() {
				var dropElm = plupload.getElement(uploader.settings.drop_element), dropInputElm, isJustAdded = false;
				...
				if (dropElm) {
					...
					if (fakeSafariDragDrop) {
						plupload.addEvent(dropElm, 'dragenter', function(e) {
							...
							if (!dropInputElm) {
	                            ...
	                            // ATLASSIAN - the act of inserting the input element fires an ondragleave event
	                            // This flag ensures that the input isn't removed straight away.
	                            isJustAdded = true;
							}
                        ...
                        // ATLASSIAN: Ensure the input is removed if the drag leaves the drag area
                        // e.g. no drop done, or dropped into another element.
                        plupload.addEvent(dropElm, 'dragleave', function(e) {
                            if (!isJustAdded) {
                                dropInputElm && dropInputElm.parentElement.removeChild(dropInputElm);
                                dropInputElm = null;
                            }
                            isJustAdded = false;
                        });
					...
					plupload.addEvent(dropElm, 'drop', function(e) {
                        /*
                        ATLASSIAN: disable page attachment uploader handler if an element
                        exists on the page with a "disable-attachment-uploader" class

                        TODO: this needs to be updated/fixed with a better approach
                              to dealing with the need to disable page attachment uploads
                              in particular instances (e.g. when a modal dialog is open).
                              Refer to CONFDEV-26619.
                        */
                        if (!$('.disable-attachment-uploader').length) {
                            // Add dropped files
                        ...
                        //ATLASSIAN
                        //the event handler seems to be a little odd when it comes to which one it will hit first.
                        //either way we want it removed.
                        dropInputElm && dropInputElm.parentElement.removeChild(dropInputElm);
                        dropInputElm = null;
                        e.preventDefault();
                        // ATLASSIAN: plupload has no real way to create more than one handler, so an event bound to the body causes issues
                        if (uploader.settings.stop_propagation) {
                            if (e.cancelBubble) {
                                e.cancelBubble = true;
                            } else {
                                e.stopPropagation();
			...
			uploader.bind("UploadFile", function(up, file) {
				...
				function sendBinaryBlob(blob) {
					...
					function uploadNextChunk() {
						...
						function prepareAndSend(bin) {
								...
								if (xhr.readyState == 4) {
									...
									if (httpStatus >= 400) {
										up.trigger('Error', {
											...
											file : file,
											status : httpStatus,
                                            //ATLASSIAN: added xhr and response so we can actually deal with some useful messages
                                            xhr : xhr,
							...
							// used by image paste in clipboard-access.js
							if(bin.encoding){
								xhr.setRequestHeader('Content-Encoding', bin.encoding);
							}
							// Set custom headers
							plupload.each(up.settings.headers, function(value, name) {
								xhr.setRequestHeader(name, value);
							...
							xhr.send(bin.getData ? bin.getData() : bin);
						} // prepareAndSend
			    ...
			    // ATLASSIAN this if is taken in Firefox 3.6 but fails by only upload the first 8 bytes of a file(?)
