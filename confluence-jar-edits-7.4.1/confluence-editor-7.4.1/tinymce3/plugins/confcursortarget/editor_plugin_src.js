/* eslint-disable @atlassian/confluence-server/matching-tests */
/**
    ...
    return {
        ...
        cursorTargetPlugin: {
            ...
            init: function(ed) {
                 ...
                 *
                 * @return {boolean} true if the cursor is at the end of the container.
                 */
                ...
                function shouldCancelBackspace(selectedNode, range) {
                    ...
                    if (tinymce.isWebKit
                        ...
                        return true;
                    } if (tinymce.isGecko && isCursorAtStartOfContainer(range) && isCursorTarget(selectedNode) && containsSoloBrElement(selectedNode)) {
                        return true;
                    }
                ...
                var isCollabEditingEnabled = function() {
                    function isOnEditScreen() {
                        ...
                        return (Meta.get('content-type') === 'page' || Meta.get('content-type') === 'blog');
                    }

                    var sharedDraftsEnabled = Meta.get('shared-drafts');
                    return sharedDraftsEnabled && isOnEditScreen() && Meta.get('edit-mode') === 'collaborative';
                };
