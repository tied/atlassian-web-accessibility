...
OC.ControlView = Backbone.View.extend({
    ...
    events: {
        ...
        "click .edit-in-office": "edit" // .edit class is only for edit in office button, won't attach for edit with companion
    },
    ...
    render: function() {
        ...
        var $placeHolder = this.$el.find('.companion-edit-button-placeholder');
        try {
            var embeddedInitializer = require('cp/component/companion/embedded-edit-with');
        } catch (e) {
            AJS.warn('Embedded Edit With resource not available', e);
        }
        if (embeddedInitializer && embeddedInitializer.loadEditButton && $placeHolder.length) {
            embeddedInitializer.loadEditButton($placeHolder).then(function(companionMainView) {
                AJS.trigger('oc-slideviewer-embedded-edit-button-loaded', companionMainView);
            }, function (err) {
                AJS.debug(err)
            });
        }
    ...
    edit: function(e) {
        ...
        editInOffice.doEditInOffice(AJS.contextPath(), this.model.get("editUrl"), "PowerPoint.Show", this.model.get("usePathAuth"));
    },
    ...
    onEnterFullscreen: function() {
        ...
        this.$(".fs .aui-icon").removeClass("aui-iconfont-vid-full-screen-on").addClass("aui-iconfont-vid-full-screen-off fs");
        this.$(".prev .aui-icon").addClass("fs");
        this.$(".next .aui-icon").addClass("fs");
        this.$(".download, .edit-in-office").hide();
    ...
    onLeaveFullscreen: function() {
        ...
        this.$(".fs .aui-icon").removeClass("aui-iconfont-vid-full-screen-off fs").addClass("aui-iconfont-vid-full-screen-on");
        this.$(".prev .aui-icon").removeClass("fs");
        this.$(".next .aui-icon").removeClass("fs");
        this.$(".download, .edit-in-office").show();
...
AJS.$(function() {
    AJS.$(".vf-slide-viewer-macro").each(function () {
        ...
        var data = {
            ...
            editInOfficeEnabled: AJS.DarkFeatures.isEnabled("enable.legacy.edit.in.office"),
            allowEdit: $this.data("allow-edit")
        };
        if (AJS.DarkFeatures.isEnabled('enable.legacy.edit.in.office')) {
